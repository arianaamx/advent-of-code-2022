...
const goThroughInstructions = (instructions, stacksObject) => {
    ...
    instructionsArray.forEach((instruction) => {
        const [moveNum, fromStack, toStack] = instruction.match(/\d+/g);

        for (let i = 0; i < moveNum; i++) {
            const pop = stacksObject[fromStack].pop();
            stacksObject[toStack].push(pop);
        }
    });
...
const puzzle1 = (data) => {
    ...
    const stackAfterInstructions = goThroughInstructions(
        instructions[0],
        stacksObject
    );
